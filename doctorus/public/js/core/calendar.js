...
frappe.views.Calendar = Class.extend({
	...
	setup_options: function() {
		...
		this.cal_options = {
			...
			eventMouseover: function(calEvent, jsEvent) {
			    var tooltip = '<div class="tooltipevent" style="background:#fefefe;border: 1px solid #aaa;border-radius:5px;position:absolute;z-index:10001;padding:10px;">' + calEvent.tooltipMessage + '</div>';
	...
	prepare_colors: function(d) {
		...
		if(this.get_css_class) {
			color_name = 
				this.color_map[this.get_css_class(d)];
			/*color_name =
