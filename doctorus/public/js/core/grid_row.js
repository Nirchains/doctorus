	...
	set_row_index: function() {
		if(this.doc) {
			this.wrapper
				...
				.find(".row-index span.number, .grid-form-row-index .number").html(this.doc.idx)
		...
		if(!this.row_index) {
			...
			this.row_index = $(
				`<div class="row-index sortable-handle col col-xs-1">
					${this.row_check_html}
				<span class="number">${txt}</span>
				<span class="octicon octicon-link-external open" title="Abrir"></span></div>`)
				.appendTo(this.row)
				.on('click', function(e) {
